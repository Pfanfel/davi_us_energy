...
map_consumption = USmapHEX_with_switch(
    "choropleth-map-consumption", "consumption-map-switch"
)
...
consumption_map_icicle_plot_container = HorizontalItemContainer(
    map_consumption, icicle_plot_consumption
)
production_map_icicle_plot_container = HorizontalItemContainer(
    map_production, icicle_plot_production
)
multi_state_switch = MultiStateSwitch("multi-state-switch")
time_slider = TimeSlider()

time_slider_checkboxes_container = HorizontalItemContainer_Custom(
    multi_state_switch, time_slider, "15%", "85%"
)
...
stacked_area_chart_percentage_consumption = StackedAreaChart(
    "stacked-area-chart-consumption"
)
stacked_area_chart_percentage_production = StackedAreaChart(
    "stacked-area-chart-production"
)

stack_area_chart_div_chart_container_con = HorizontalItemContainer_Custom(
    div_bar_chart_con, stacked_area_chart_percentage_consumption, "40%", "60%"
)
stack_area_chart_div_chart_container_pro = HorizontalItemContainer_Custom(
    div_bar_chart_pro, stacked_area_chart_percentage_production, "40%", "60%"
)
...
# STORAGE
storage_consumption_overview = dcc.Store(
    id="consumption_overview_data_storage", data=dt.stads_df.to_dict("records")
)
...
selected_category_overview_con = dcc.Store(
    id="selected_category_overview_con", data=["TETCB"]
)
...
selected_category_overview_pro = dcc.Store(
    id="selected_category_overview_pro", data=["TEPRB"]
)
